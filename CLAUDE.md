...
- `bun run test` — Vitest
- `bun run test:coverage` — Vitest with the per-layer coverage gates (what the CI
  `test` job runs; report in `coverage/`). Strategy, thresholds and the ratchet
  rule: `docs/quality/testing.md`.
